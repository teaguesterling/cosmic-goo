...
| **Type detection — the signal ladder** — cheap signals (`@type`/source `emits`/extension/Content-Type/structural/libmagic) → weighted candidates → re-ranked by `accepts`; gating only on *inferential* signals; extension covers CSV/YAML (no parser) | **designed** | [detection.md](detection.md) |
