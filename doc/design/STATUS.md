...
| **Type detection — the signal ladder** — weighted candidates (explicit override / handle `emits` / extension / Content-Type / structural / libmagic) the verb's `accepts` *selects* among ("is this usable as what I need?"); gating only on *inferential* signals; **`emits` types the handle not the content**; **detectors are *declared* in `core.toml` (`impl.cmd` primary — no privileged hardwired types)**, `impl.builtin` native registry deferred to measured need | **designed** | [detection.md](detection.md) |
