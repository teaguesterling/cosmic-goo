...
| **Type detection — the signal ladder** — weighted candidates (explicit override / handle `emits` / extension / Content-Type / structural / libmagic) the verb's `accepts` *selects* among ("is this usable as what I need?"); gating only on *inferential* signals (`is_a`/`what_is` detectors, guarded); **`emits` types the handle not the content**; extension covers CSV/YAML (no parser) | **designed** | [detection.md](detection.md) |
