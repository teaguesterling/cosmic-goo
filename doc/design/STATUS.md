...
| **Type detection — the signal ladder** — cheap signals (`--as`/source `emits`/extension/Content-Type/structural/libmagic) → weighted candidates → re-ranked by `accepts`; gating only on *inferential* signals (`is_a`/`what_is` detectors, guarded); extension covers CSV/YAML (no parser) | **designed** | [detection.md](detection.md) |
