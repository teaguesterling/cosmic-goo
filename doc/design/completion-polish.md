...
**D1. The confirm override and the `[!]` chip.** The override changes the *run-time gate*, not the verb's *nature*. → Chip ALWAYS shows when `confirm: true`, regardless of any override. Rationale: the chip is "this verb wants confirmation"; the override is "I, the caller, am suppressing it this once." Conflating them hides what the verb's TOML actually declares. **The override is `--confirm-dangerous=verb1,verb2`** — a *scoped, per-invocation* pre-approval that suppresses the interactive y/N only for the named verbs (any other confirm/destructive verb in the same run still prompts). It is deliberately **not** a blanket `--no-confirm` and deliberately **not** an env var: a persistent/global "skip all confirms" is the unsafe pattern this replaces. A run-time auto-approve note is printed so the bypass is never silent. (Earlier drafts named a `--no-confirm` flag; it was never implemented and is superseded by this.)
